<template>
  <div class="page-wzgl-wzjg-ils-detail" v-if="visible">
    <div class="header-crumb">
      <span @click="hide" style="cursor: pointer;">数据中心>全部数据>详细</span>
    </div>
    <div class="body">
      <div class="column-1">
        ...
        <el-button class="search-button" @click="searchVeh_sjzx(data)">搜索相似车辆
        ...
        <div class="line">
          ...
          <div style="display: flex">
            <div style="width: 50%">
              ...
              <div>主驾驶抽烟: <span class="value">{{data.md_isSmoke > 0 ? '是' : '否'}}</span></div>
              <div>副驾驶不系安全带: <span class="value">{{data.ct_isSafetyBelt > 0 ? '是' : '否'}}</span></div>
              <div>摩托车不系头盔: <span class="value">{{data.mc_isHelmet > 0 ? '是' : '否'}}</span></div>
            </div>
            <div style="width: 50%">
  ...
  export default {
    ...
    methods: {
      searchVeh_sjzx(data){
        ...
        util.bus.$emit('goSearchVeh_sjzx',params);
      },
      hide(){
