  ...
  export default {
    ...
    methods: {
      onRowClick(row, column, event){
        console.log('cameraId=' + row.diId + '!!!!!!!!!!!! store:' + JSON.stringify(this.$store.state) + '!')
        this.$store.commit("setCameraId", 101)
        console.log('??????????????????????????? OK ??????????????????????????')
        console.log('##### cameraId=' + this.$store.stp.video_analysis.state.cameraId + '!!!!!!!!!!!!!!!!!!!!!!!!')
